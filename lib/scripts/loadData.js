const { getCollection, log, tpClient } = require('../helpers');
...
async function load(entity) {
  ...
  if (properties.count === 0) {
    ...
    await importData(data, coll, type, endpoints);
  }
  else {
...
async function importData(data, coll, type, endpoints) {
  ...
  log('info', `Loading ${coll.name} into database...`);
  await coll.save(data, { silent: true });
  log('info', `Loaded ${data.length} ${coll.name} into database.`);
}
