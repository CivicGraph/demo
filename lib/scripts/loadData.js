...
(async () => {
  ...
  const links = [
    ['airports', 'cities', 'city_code', 'airports-cities'],
    ['cities', 'countries', 'country_code', 'cities-countries']
  ];
...
async function link(childCollName, parentCollName, parentField, linkCollName) {
  const childColl = getCollection(childCollName, 'vertex');
  const parentColl = getCollection(parentCollName, 'vertex');
